...
def _retrieve_context(
	*,
	...
	def _format_chunks(chunks: list[dict[str, Any]]) -> dict[str, Any]:
		context_parts = []
		sources = []
		for chunk in chunks:
			...
			for key in ("ticker", "filing_type", "year", "quarter", "section", "source"):
				value = chunk.get(key)
				if value:
					source[key] = str(value) if key == "year" else value
			chunk_id = chunk.get("chunk_id") or chunk.get("id")
			if chunk_id:
				source["chunk_id"] = str(chunk_id)
	...
	if retrieval_mode == "combined":
		data = HybridRetriever(strict_hybrid=False).combined_search(
			question,
			top_k=top_k,
	...
	if retrieval_mode == "hybrid":
		chunks = HybridRetriever(strict_hybrid=False).retrieve(
			question,
			top_k=top_k,
			hybrid=True,
			metadata_filter=metadata_filter,
	...
	if retrieval_mode == "graph_local":
		query_embedding = HybridRetriever(strict_hybrid=False)._embed(question)
		data = GraphRetriever().local_search(query_embedding=query_embedding, top_k=top_k, hop=2)
		return {
			"context_text": data.get("context_text", ""),
			"sources": [{"chunk_id": str(cid), "retrieval_source": "graph_local"} for cid in data.get("chunk_ids", [])],
			"stats": {"chunk_ids": len(data.get("chunk_ids", []))},
		}

	if retrieval_mode == "graph_global":
		query_embedding = HybridRetriever(strict_hybrid=False)._embed(question)
		data = GraphRetriever().global_search(query_embedding=query_embedding, top_k=top_k)
		return {
			"context_text": data.get("context_text", ""),
			"sources": [{"chunk_id": str(cid), "retrieval_source": "graph_global"} for cid in data.get("chunk_ids", [])],
			"stats": {"chunk_ids": len(data.get("chunk_ids", []))},
		}
