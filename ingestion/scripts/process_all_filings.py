...
def main():
    ...
    parser.add_argument(
        "--ticker",
        action="append",
        help="Only process filings whose filename starts with this ticker. Can be repeated.",
    )
    parser.add_argument(
        "--include-cleaned",
        action="store_true",
        help="Also process *_clean.htm files. Off by default to avoid duplicate chunks.",
    )
    ...
    if not args.include_cleaned:
        htm_files = [
            path
            for path in htm_files
            if not path.stem.endswith("_clean") and not path.stem.endswith("_clean_clean")
        ]
    if args.ticker:
        allowed_tickers = {ticker.upper() for ticker in args.ticker}
        htm_files = [
            path
            for path in htm_files
            if path.name.split("_", 1)[0].upper() in allowed_tickers
        ]
    if not htm_files:
